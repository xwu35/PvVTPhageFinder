...
rule host_filter_mapped_bam:
    """because there is no prophage region, so remove reads mapped to non-prophage region >= 98% identity"""
    input:
        mapped=os.path.join(dir["output"]["intermediate"], "host_filtered", "{sample}_mapped_sorted.bam")
    output:
        mappedFiltered=os.path.join(dir["output"]["intermediate"], "host_filtered", "{sample}_mapped_sorted_filtered.bam")
    threads:
        config["resources"]["med_cpu"]
    params:
...
rule host_merge_bam_files:
    """merge unmapped and non-prophage filtered bam files"""
    input:
        unmapped=os.path.join(dir["output"]["intermediate"], "host_filtered", "{sample}_unmapped_sorted.bam"),
        mappedFiltered=os.path.join(dir["output"]["intermediate"], "host_filtered", "{sample}_mapped_sorted_filtered.bam")
    output:
